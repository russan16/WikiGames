        ...
        return (
            <div className="mt-5 mb-5 flex-wrap">
                ...
                {this.state.plataformas.map(plat =>(
                    <a href={`/plataforma-games/${plat.id}/${plat.name}`} className="btn btn-secondary mr-2 mb-2">{plat.name} <span className="badge badge-dark badge-pill">{plat.games_count}</span></a>
                ))}
            </div>
        );
    }
